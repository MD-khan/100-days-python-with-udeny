...
machine_pick_city = random.choice(city_list).lower()

display = []
# making slots to put the letter
# We creating slot putnong the _
for under_score in range(len(machine_pick_city)):
    display += "_"
...
while end_game != True:
    ...
    for position in range(len(machine_pick_city)):
        letter = machine_pick_city[position]
        if letter == guess:
            display[position] = letter
    if guess not in machine_pick_city:
        lives -= 1
        if lives == 0:
            end_game = True
            print("Try Again!")
            print(f"The city was {machine_pick_city.upper()}")
    # print(display)
